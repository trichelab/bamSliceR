...
getGDCBAMs = function(project_id = "", es = "" , workflow = "" )
{
    stopifnot("project_id not exists" = (project_id %in% results_all (projects())$id))

    fields = available_fields("files") [str_detect(available_fields('files'), "submitter|bios|sample_type|workflow")]

    files() %>% GenomicDataCommons::select(c(fields,"file_name")  ) %>%
        GenomicDataCommons::filter( cases.project.project_id == cohort ) %>%
        GenomicDataCommons::filter( experimental_strategy == es  ) %>%
        GenomicDataCommons::filter( data_format == "BAM") -> qfiles
    if (workflow != "")
    {
        qfiles = qfiles %>% GenomicDataCommons::filter( analysis.workflow_type == workflow)
    }

    file_meta = results_all(qfiles)

    case_id = sapply(file_meta$cases, function(x) { return(x$submitter_id) } )
    sample_types = sapply(file_meta$cases, function(x) { return(x$samples[[1]]$sample_type) } )
    id_case_match = with(file_meta, cbind(id, sample = unname(unlist(associated_entities) ) , file_name ))
